...
type ActivityListResponse = Activity[] | { content?: Activity[]; activities?: Activity[]; data?: Activity[] };
...
export const activityApi = {
  ...
  async timeline(schoolId: string, page = 0, size = 80) {
    const payload = await apiClient<ActivityListResponse>('/api/feed/timeline', {
      schoolId,
      query: { page, size },
    });
    return normalizeList(payload);
  ...
  async pending(schoolId: string) {
    const payload = await apiClient<ActivityListResponse>('/api/activities/pending', { schoolId });
    return normalizeList(payload);
  },
  ...
  submit(schoolId: string, activityId: string | number, teacher = false) {
    ...
    return apiClient<Activity>(`${prefix}/${activityId}/submit`, { method: 'POST', schoolId });
  },

  approve(schoolId: string, activityId: string | number, remarks = 'Approved for publishing.') {
  ...
  publish(schoolId: string, activityId: string | number) {
    return apiClient<Activity>(`/api/activities/${activityId}/publish`, { method: 'POST', schoolId });
  },
};
