  ...
  return (
    <main className="page-dark" style={{ minHeight: '100vh', padding: 24 }}>
      ...
      <section className="premium-panel" style={{ position: 'relative', zIndex: 1, maxWidth: 980, margin: '0 auto', borderRadius: 30, padding: 28 }}>
        <p className="eyebrow">SCHOOL ACTIVATION & WORKSPACE PROVISIONING</p>
        ...
        {pkg ? (
          <div style={{ display: 'grid', gap: 14, marginTop: 16 }}>
            ...
            {pkg.workspaceSteps?.length ? <div className="notice-card"><strong>Workspace Provisioning</strong><div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: 10, marginTop: 12 }}>{pkg.workspaceSteps.map((step) => <div key={step.key} className="secondary-button" style={{ pointerEvents: 'none', display: 'block', textAlign: 'left', opacity: step.status === 'DONE' ? 1 : 0.72 }}><strong>{step.status}</strong><br />{step.label}<br /><span style={{ fontSize: 12, opacity: 0.82 }}>{step.detail}</span></div>)}</div></div> : null}
            ...
            {pkg.activationChecklist?.length ? <div className="notice-card"><strong>Activation Handover Checklist</strong><ul style={{ margin: '10px 0 0', paddingLeft: 18 }}>{pkg.activationChecklist.map((item) => <li key={item}>{item}</li>)}</ul></div> : null}
            {pkg.importPreparationChecklist?.length ? <div className="notice-card"><strong>Excel Import Preparation</strong><ul style={{ margin: '10px 0 0', paddingLeft: 18 }}>{pkg.importPreparationChecklist.map((item) => <li key={item}>{item}</li>)}</ul></div> : null}
